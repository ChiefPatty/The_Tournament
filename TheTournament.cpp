...
// PROMPTS TO USER
void game_boot_prompt(int& menuInput);
void new_game_message(Weapon equipped_weapons[], User& Player); //Function to present new game menu to menu

// GAME MENUS
void tournament_hub_menu(Weapon equipped_weapons[], Armor equipped_armor[], Potion equipped_potions[], User Player);
void shopMenu(Weapon equipped_weapons[], Armor equipped_armor[], Potion equipped_potions[], User Player);
void shipMenu(Map map, Weapon equipped_weapons[], Armor equipped_armor[], Potion equipped_potions[], User Player);

// GENERATE RANDOM WEAPON
string generate_weapon_name();
int generate_weapon_base_damage(int level);
int generate_weapon_max_damage(int base_damage);
int generate_weapon_cost(int base_damage, int max_damage); 

//GENERATE RANDOM ARMOR

//HELMET
string generate_helmet_name();
int generate_helmet_health(int level);
int generate_helmet_cost(int health);

// CHEST PIECE GENERATION
string generate_chest_name();
int generate_chest_health(int level);
int generate_chest_cost(int health);

// BOOTS GENERATION
string generate_boots_name();
int generate_boots_health(int level);
int generate_boots_cost(int health);

// GENERATE RANDOM OPPONENT
string generate_opponent_name();
string generate_opponent_backstory();
string generate_opponent_attack_name();
int generate_opponent_level(int level);

// DISPLAY STATS
void displayPlayerStats(User Player);
void displayWeaponStats(int input, Weapon equipped_weapons[]); // Call to display stats of currently equiiped weapons
void displayArmorStats(int input, Armor equipped_armor[]);
void displayPotions(int input, Potion equipped_potions[]);
...
{   
    ...
    game_boot_prompt(menuInput);
    
    // Run if user is starting a game from scratch
    if  (menuInput == 1) 
    {
        generate_weapon_base_damage(level);
        new_game_message(equipped_weapons, Player);
        tournament_hub_menu(equipped_weapons, equipped_armor, equipped_potions, Player);
    }
    // Run if user is loading a previous save
    else 
    {
        // Open File Stream to Get Old Attributes and gear to user
        /**
         * Use Split function to separate out fields
        */
        tournament_hub_menu(equipped_weapons, equipped_armor, equipped_potions, Player);
        return 0;
    }
...
// BOOT UP PROMPTS
void game_boot_prompt(int& menuInput) // Asks user if they have a previous save
{
    menuDivider();

    cout << "Welcome to the Tournament! Are you starting a new game or loading a playthrough?" << endl << endl;
    cout << "1. [NEW GAME]" << endl;
    cout << "2. [LOAD] - NOT AVAILABLE YET" << endl << endl;
    ...
    {
        menuDivider();
        cout << "Please enter the number 1. Load is still a work in progress [I'll resend Monday]" << endl;
        cout << "1. [NEW GAME]" << endl;
        cout << "2. [LOAD]" << endl << endl;

        cout << "Menu Selection: "; cin >> menuInput;
    }
}
void new_game_message(Weapon equipped_weapons[], User& Player) // If new game this outputs the story and gives user starting gear
{
    string username;
...
void tournament_hub_menu(Weapon equipped_weapons[], Armor equipped_armor[], Potion equipped_potions[], User Player) // Main Navigation Function of Program
{
    ...
    else if (menuInput == 5)
    {
        ...
        if (menuInput == 1)
        {
            // Ask user if they would like to save progress
            cout << "Would you like to save your progress?" << endl << endl;
            ...
            if (menuInput == 1)
            {
                string filename; //Create temporary string to save filename to
                //Ask user what they would like to name the save file
                cout << "What would you like to save the file as?" << endl << endl;
                cout << "Game File Name: "; cin >> filename;

                filename = "Game_Saves/" + filename + ".txt";

                ofstream Save;
                Weapon Primary(equipped_weapons[0]);
                Weapon Secondary(equipped_weapons[1]);

                Armor Helmet(equipped_armor[0]);
                Armor Chest(equipped_armor[1]);
                Armor Boots(equipped_armor[2]);
                
                Save.open(filename); //Works up to here
                Save << "Name|" << Player.getUserName() << "|Level|" << Player.getUserLevel() << "|XP|" << Player.getUserXP() << "|" << endl;
                Save << "Credits|" << Player.getUserCredits() << "|" << endl;
                Save << "Weapons|Primary|" << Primary.getWeaponName() << "|Secondary|" << Secondary.getWeaponName() << "|" << endl;
                Save << "Armor|Helmet|" << Helmet.getArmorName() << "|Chest|" << Chest.getArmorName() <<"|Boots|" << Boots.getArmorName() << "|" << endl; 
                Save.close();

                menuDivider();

                cout << "The Game Save: " << filename << " was stored to the Game_Saves folder" << endl;
...
// HELMET GENERATION
string generate_helmet_name() // Returns a random name from weapons name list
{
    int random = generate_random_number(0, 99);
...
int generate_helmet_cost(int health) // Return a weapon cost based on damage
{
...
// CHEST PIECE GENERATION
string generate_chest_name() // Returns a random name from weapons name list
{
    int random = generate_random_number(0, 99);
...
int generate_chest_cost(int health) // Return a weapon cost based on damage
{
...
// BOOTS GENERATION
string generate_boots_name() // Returns a random name from weapons name list
{
    int random = generate_random_number(0, 99);
...
int generate_boots_cost(int health) // Return a weapon cost based on damage
{
...
string generate_opponent_attack_name() // Returns a random attack type from the atatck list
{
    int random = generate_random_number(0, 99);
...
void displayWeaponStats(int input, Weapon equipped_weapons[]) //Function to display all armor stats
{
    ...
    cout << "Hits Remaining: " << equipped_weapons[input].getWeaponHitsRemaining() << endl;
    cout << "Upgrades Remaining: " << equipped_weapons[input].getWeaponUpgradesRemaining() << endl;
}
void displayArmorStats(int input, Armor equipped_armor[]) //Function to display all armor stats
{
    ...
    cout << "Hits Remaining: " << equipped_armor[input].getArmorHitsRemaining() << endl;
    cout << "Upgrades Remaining: " << equipped_armor[input].getArmorUpgradesRemaining() << endl;
}
void displayPotions(int input, Potion equipped_potions[]) //Function to display all armor stats
{
